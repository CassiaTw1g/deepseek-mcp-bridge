...
- **The README now opens with what the bridge is actually for.** One sentence and a four-row table — a native sub-agent gives advice from inside the vendor's sandbox on a ~60 s budget; this reads and writes files on your machine, runs commands, and works an asynchronous job to completion. The measured run is cited as the evidence, with the default-off warning directly beneath it.
...
- **README: what a host-model swap does and does not change.** Both READMEs now state that the bridge's value comes from two platform constraints (no filesystem access for a hosted sub-agent, ~60 s per tool call), not from which tier the caller runs — so changing tiers leaves it doing the same job. Re-pointing the sub-agent side is two `.env` lines (`DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL`) plus `reload`. The only changes that would make it redundant are platform-level, and are named.
