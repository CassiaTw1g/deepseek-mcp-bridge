...
**它让 ChatGPT 从「告诉你怎么做」,变成「在你电脑上把活干完」。**

原生 ChatGPT 子代理做不到这件事:它们跑在厂商沙箱里,碰不到你的文件系统;而且每一次工具调用都受约 60 秒的预算限制,长任务会被硬生生砍断。这个桥接把这两堵墙一起拆了。

**关键在于它是怎么做到的:不是把 DeepSeek 接进来,而是给它配了一副手。** 一个只转发 API 的桥接,给宿主的只是「又一个可以问的模型」。这里不是——收到 agent 任务时,桥接会以子进程方式启动**一整套 agent harness**([Claude Code](https://claude.com/claude-code)),把它的端点指向 DeepSeek 的 Anthropic 兼容接口。于是**循环、上下文压缩、提示缓存、工具实现**——所有「让模型真能干活」的部分——用的是现成的、有人在维护的实现,而不是手搓一个仿制品。宿主拿到的因此是一个**能自己干活的子代理**,而不是一个问答接口。

而且它对外只是**一个 MCP 工具**:任何支持 MCP 的宿主都能调用,ChatGPT connector 只是其中一个入口(目前实测过的只有 ChatGPT)。harness 这一层是可替换的部件——Claude Code 是它当前的实现,不是这个设计的本体。

| 原生子代理 | 本桥接 |
|---|---|
| 只能给建议,动手的是你 | **读、写、改你机器上的文件** |
| 跑在厂商沙箱里 | **在你的目录里执行命令,多步迭代到任务做完** |
| 单次工具调用约 60 秒上限 | **异步任务,可以跑十几分钟** |
| 你看不见它将要做什么 | **每条名单外的命令都暂停,等你批准** |

**实测,不是设计意图**:给它「读取 witness.txt,把内容反转后写入 answer.txt」,它跑了 3 步、2 分 25 秒,磁盘上的 `answer.txt` 与预期字符串**逐字节一致**,并报出了只有真实结果里才会出现的验证码。聊天记录自己跟自己一致不算证据,**磁盘上的字节才算**。

> ⚠️ **这个能力默认是关的。** 不设 `DEEPSEEK_ALLOWED_ROOTS`,它只能花你的 DeepSeek 额度,碰不到你的电脑。一旦打开,拿到 URL 的人就能在你的机器上读写文件、执行命令——**「能执行命令」等于「拿到了这台电脑」**。先读[安全模型](#安全模型)。
...
**换主模型不影响它。** 桥接不依赖 Sol。它的价值来自两条平台约束——宿主子代理没有文件系统访问、一次工具调用只有约 60 秒——这两条与模型档位无关。换到别的档位,两堵墙还在,桥接就还在做同一件事。需要重新指向的只有子代理那一侧:改 `.env` 里的 `DEEPSEEK_BASE_URL` 和 `DEEPSEEK_MODEL`,再 `npm run ctl -- reload`(隧道不动,URL 不变)。真正会让它变得多余的是两件**平台级**的事,不是模型换代:给子代理开放文件系统访问,或把工具调用预算提到分钟级。
