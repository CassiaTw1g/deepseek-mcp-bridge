...
const MESSAGES = {
  zh: {
    ...
    "q.tunnel.tokenExplain": [
      ...
      "  (老的 Zero Trust → Networks → Connectors 菜单现在会跳到这儿,是同一个页面)",
      "  起个名字 → 下一步 → 页面上会给一串很长的 token。",
      "  (已经建好了?点进那条隧道 → Configure,token 也在那里)",
    ],
    ...
    "q.tunnel.dnsNote": [
      ...
      "     Networking → Tunnels → 这条隧道 → Routes → Add route →",
      "     Published application(已发布的应用)",
      "     子域名 {sub} / 域 {domain} / Service 选 HTTP / URL 填 http://localhost:{port}",
  ...
  en: {
    ...
    "q.tunnel.tokenExplain": [
      ...
      "  Networking → Tunnels → Create a tunnel",
      "  (the older Zero Trust → Networks → Connectors menu redirects here now —",
      "  same page). Name it, go to the next step, and the page shows a long token.",
      "  (Already created it? Open the tunnel → Configure and it is there too.)",
    ],
    ...
    "q.tunnel.dnsNote": [
      ...
      "     Networking → Tunnels → this tunnel → Routes → Add route →",
      "     Published application",
      "     subdomain {sub} / domain {domain} / service HTTP / URL http://localhost:{port}",
